...
def export_json():
    omraade, snitt, min, max, time_now = get_lists()
...
while True:
    ...
    # if current time is between full hour and 5 minutes after full hour.
    if current_time[-2:] == "00" or current_time[-2:] == "01" or current_time[-2:] == "02" or current_time[-2:] == "03" or current_time[-2:] == "04": 
        try:
            export_json()
            get_time = datetime.datetime.now().strftime("%H:%M:%S")
            print(f"[{get_time}] JSON file updated.")
            log_success()
            time.sleep(300)
        except:
            log_error()
            get_time = datetime.datetime.now().strftime("%H:%M:%S")
            print(f"[{get_time}] Error while updating JSON file.")
        ...
        get_time = datetime.datetime.now().strftime("%H:%M:%S")
        print(f"[{get_time}] Waiting for next update..")
    
    waiting_animation(100)
